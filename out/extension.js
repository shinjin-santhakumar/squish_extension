...
async function updatePylancePaths(userDirs, stubsDir) {
    try {
        ...
        const overrides = pythonConfig.get("diagnosticSeverityOverrides") ?? {};
        if (overrides["reportUndefinedVariable"] !== "none") {
            await pythonConfig.update("diagnosticSeverityOverrides", { ...overrides, reportUndefinedVariable: "none" }, vscode.ConfigurationTarget.Workspace);
        }
